def insert_into_all(item, nested_list):
    ...
    [[0], [0, 1, 2], [0, 3]]
    """
    return [[item] + sublst for sublst in nested_list]
...
def trade(first, second):
    ...
    while (dif := equal_prefix()) != 0 and m < len(first) and n < len(second):
        if dif < 0:
            m += 1
        else:
            n += 1
    print("DEBUG:", m, n)
    if not equal_prefix() and m < len(first) and n < len(second):
...
def make_glookup(class_assignments):
    ...
    total_score = 0
    full_score = 0
    def percentage(name, score):
        nonlocal  total_score, full_score
        total_score += score
        full_score += class_assignments[name]
        return float(total_score / full_score)
...
def make_advanced_counter_maker():
    ...
    def make_counter():
        personal_counter = 0
        def counter(ins):
            nonlocal personal_counter, global_count
            "*** YOUR CODE HERE ***"

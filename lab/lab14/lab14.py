def prune_min(t):
    ...
    if t.is_leaf():
        ...
        ls, rs = t.branches[0], t.branches[1]
        tar = ls if ls.label <= rs.label else rs
        prune_min(tar)
        t.branches = [tar]
        return
...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx, res, cost):
        """
        ...
        if skeleton_idx < len(skeleton) and code_idx == len(code):
            ...
            return res + edits, cost + len(skeleton) - skeleton_idx
        if skeleton_idx == len(skeleton) and code_idx < len(code):
            edits = "".join(["+[" + c + "]" for c in code[code_idx:]])
            return res + edits, cost + len(code) - code_idx

        possibilities = []
        skel_char, code_char = skeleton[skeleton_idx], code[code_idx]
        # Match
        if skel_char == code_char:
            res, cost = helper_align(skeleton_idx + 1, code_idx + 1, res + skel_char, cost)
            possibilities.append((res, cost))
        # Insert
        res2, cost2 = helper_align(skeleton_idx, code_idx + 1, res + "+[{}]".format(code_char), cost + 1)
        possibilities.append((res2, cost2))
        # Delete
        res1, cost1 = helper_align(skeleton_idx + 1, code_idx, res + "-[{}]".format(skel_char), cost + 1)
        possibilities.append((res1, cost1))
        return min(possibilities, key=lambda x: x[1])

    result, cost = helper_align(0, 0, '', 0)
    return result

...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        for idx in range(len(opponent.deck.cards)-1, -1, -1):
            card = opponent.deck.cards[idx]
            if card.attack == other_card.attack or card.defense == other_card.defecse:
                opponent.deck.drop(idx)

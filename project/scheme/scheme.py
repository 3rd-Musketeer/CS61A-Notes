...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    #print("DEBUG:", scheme_symbolp(first), repr(first))
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if cur1 != cur2:
            raise SchemeError("Number of arguments and parameters doesn't match!")
        return new_frame
        # END PROBLEM 10
...
class BuiltinProcedure(Procedure):
    ...
    def apply(self, args, env):
        ...
        #print("DEBUG: applying {} on {}".format(self.fn, python_args))
        try:
            return self.fn(*python_args)
        except TypeError as e:
            raise SchemeError("Wrong number of arguments of {}".format(len(python_args)))
        # END PROBLEM 3
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        env.define(func_name, do_lambda_form(Pair(target.rest, expressions.rest), env))
        return func_name
        # END PROBLEM 9
    else:

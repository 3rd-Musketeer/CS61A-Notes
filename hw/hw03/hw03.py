...
def g_iter(n):
    ...
    rem_g = [1, 2, 3]
    for i in range(3, n):
        rem_g.append(rem_g[i-1] + 2 * rem_g[i-2] + 3 * rem_g[i-3])
    return rem_g[n-1]
...
def missing_digits(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    def helper(n, desired_digit):
        if n == 0:
            return 0
        if (n % 10) == desired_digit:
            return helper(n // 10, desired_digit - 1)
        elif (n % 10) > desired_digit:
            return helper(n // 10, desired_digit)
        else:
            return helper(n, desired_digit - 1) + 1
...
def count_change(total):
    ...
    def helper(total, base):
        if total == 0:
            ...
            return (total >= base and helper(total - base, base)) + helper(total, base >> 1)
    return helper(total, 1024)

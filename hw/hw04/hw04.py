...
def balanced(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def balance_helper(m):
        if is_planet(m):
            return True, size(m)
        elif is_mobile(m):
            l, r = left(m), right(m)
            lb, lw = balance_helper(end(l))
            if not lb: return False, 0
            rb, rw = balance_helper(end(r))
            if not rb: return False, 0
            return lw * length(l) == rw * length(r), lw + rw
        else:
            return False, 0

    ans, _ = balance_helper(m)
    return ans
...
def quadratic(x, a, b, c):
    ...
    lb, rb = f(lower_bound(x)), f(upper_bound(x))
    min_pnt = -b / (2 * a)
    min_val = f(min_pnt)
    if lower_bound(x) < min_pnt < upper_bound(x):
        if a > 0: return interval(min_val, max(lb, rb))
        else: return interval(min(lb, rb), min_val)
    else:
        return interval(min(lb, rb), max(lb, rb))

    ...
    def merge_generater():
        ...
        while next_a is not None or next_b is not None:
            ...
            if next_a is None:
                ...
                res = next_b
            while next_a == res: next_a = next(iter_a, None)
            while next_b == res: next_b = next(iter_b, None)
            print("DEBUG:", next_a, next_b)
            yield res

    return merge_generater()
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    "Too many incorrect attempts. Attempts: ['my', 'secret', 'password']"
    """
    def new_access(amount, password):
        if password == new_pass:
            return withdraw(amount, old_pass)
        else:
            return withdraw(amount, password)
    if type(res := withdraw(0, old_pass)) is not int:
        return res
    else:
        return new_access
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def generate_remainders():
        rem = 0
        def gen():
            num = m if rem == 0 else rem
            while True:
                yield num
                num += m

...
class VendingMachine:
    ...
    def enough_funds(self):
        return self.fund >= self.price

    def get_change(self):
        return self.fund % self.price

    def get_gap(self):
        return self.price - self.fund
    ...
    def vend(self):
        ...
        elif self.enough_funds():
            changes = self.get_change()
            self.stock -= self.num_sold()
            self.fund = 0
            if changes:
...
def is_bst(t):
    ...
    def helper(t):
        if t.is_leaf():
            ...
            num_sons = len(t.branches)
            if num_sons > 2:
                return (0, 0), False
            elif num_sons == 1:
                son = t.branches[0]
                (mn, mx), flag = helper(son)
                return (min(t.label, mn), max(t.label, mx)), flag and ((mx <= t.label) or (t.label < mn))
            else:
                ls, rs = t.branches[0], t.branches[1]
                (mn1, mx1), flag1 = helper(ls)
                (mn2, mx2), flag2 = helper(rs)
                return (mn1, mx2), (mx1 <= t.label < mn2) and flag1 and flag2
...
def path_yielder(t, value):
    ...
    [[0, 2], [0, 2, 1, 2]]
    """
    def path_finder(t, path):
        print("DEBUG:", "at", t.label)
        path.append(t.label)
        print("DEBUG:", t.label, path)
        if t.label == value:
            yield path[:]
        for b in t.branches:
            print("DEBUG:", "going to", b.label)
            yield from path_finder(b, path)
        path.pop()
    "*** YOUR CODE HERE ***"
